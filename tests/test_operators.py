...
@pytest.mark.task0_2
@given(lists(small_floats, min_size=2, max_size=10))
def test_sigmoid(a: List[float]) -> None:
    ...
    * It is  strictly increasing.
    ...
    for i, num in enumerate(a):
        assert 0 <= sigmoid(num) <= 1
        assert_close(1 - sigmoid(num), sigmoid(-num))
        assert last <= sigmoid(num)
        last = sigmoid(num)
